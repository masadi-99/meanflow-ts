"""
Train MeanFlow-TS with dual-axis self-consistency loss.
Usage: python train_dual_axis.py <dataset> --epochs 600 --lambda-temporal 0.1
"""
...
from meanflow_ts.dual_axis import dual_axis_meanflow_loss
...
# Baseline: MeanFlow v4 without temporal equivariance
V4_CRPS = {
...
def main():
    ...
    logger.info(f"=== {name} | dual-axis (λ={args.lambda_temporal}) | ctx={ctx_len} pred={pred_len} ===")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            loss, mf_val, te_val = dual_axis_meanflow_loss(
                net, scaled_future, ctx_with_lags, lambda_temporal=args.lambda_temporal,
            )
            optimizer.zero_grad()
            loss.backward()
        ...
        if (epoch+1) % 20 == 0 or epoch == 0:
            logger.info(f"Epoch {epoch+1:>3}/{args.epochs} | MF: {epoch_mf/n_b:.4f} TE: {epoch_te/n_b:.6f} | {elapsed:.1f}s")
        ...
        if (epoch+1) % 100 == 0 or (epoch+1) == args.epochs:
            ...
            if crps < best_crps:
                best_crps = crps
                torch.save({'net_ema': net_ema.state_dict(), 'epoch': epoch+1, 'crps': crps},
                           f'best_dual_{name}.pt')

            v4 = V4_CRPS.get(name, "?")
            tsflow = TSFLOW_CRPS.get(name, "?")
            logger.info(f"  CRPS={crps:.6f} | ND={nd:.6f}{tag}")
            logger.info(f"  v4 baseline: {v4} | TSFlow: {tsflow} | Best: {best_crps:.6f}")
